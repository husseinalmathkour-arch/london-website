      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="bg-gray-900 border border-gray-800 text-white text-sm rounded-xl pl-9 pr-4 py-2 focus:outline-none focus:border-blue-500 w-48" />
          </div>
              ...
              <div key={p.id} className="px-5 py-4 flex items-center gap-4 hover:bg-gray-800/20">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {p.category && <span className="text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded-full">{p.category}</span>}
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.published ? 'bg-green-500/20 text-green-300' : 'bg-gray-700 text-gray-400'}`}>{p.published ? 'Published' : 'Draft'}</span>
                  </div>
                  {p.title_tr && <p className="text-gray-500 text-xs">{p.title_tr}</p>}
                  <p className="text-gray-600 text-xs mt-0.5">{p.author && `By ${p.author} · `}{new Date(p.created_at).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => togglePublish(p.id, p.published)} className="text-gray-500 hover:text-white p-1.5 hover:bg-gray-800 rounded-lg transition-colors" title={p.published ? 'Unpublish' : 'Publish'}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-sm text-center">
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 rounded-xl border border-gray-700 text-gray-300 hover:text-white text-sm transition-colors">Cancel</button>
              <button onClick={() => deletePost(deleteConfirm)} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                {tab === 'en' && (
                  <>
                    <div>
                      ...
                      <input className={inp} value={form.title_en} onChange={e => { setForm(f => ({ ...f, title_en: e.target.value, slug: editing ? f.slug : autoSlug(e.target.value) })) }} placeholder="Post title in English" />
                    </div>
                    <div>
                      <label className={lbl}>Excerpt (EN)</label>
                      <textarea className={inp} rows={2} value={form.excerpt_en} onChange={e => setForm(f => ({ ...f, excerpt_en: e.target.value }))} placeholder="Short description" />
                    </div>
                    <div>
                      <label className={lbl}>Content (EN)</label>
                      <textarea className={inp} rows={8} value={form.content_en} onChange={e => setForm(f => ({ ...f, content_en: e.target.value }))} placeholder="Post content..." />
                    </div>
                  </>
                )}
                {tab === 'tr' && (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className={inp} value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} placeholder="Author name" />
                  </div>
                  <ImageUpload
                    label="Cover Image"
                    folder="blog"
                    ...
                    hint="Blog cover shown on the listing and at the top of the post. Recommended: 1200×630px landscape, JPG, under 1MB. Avoid portraits or faces as the main subject."
                  />
                </div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setModal(false)} className="px-4 py-2 rounded-xl border border-gray-700 text-gray-300 hover:text-white text-sm transition-colors">Cancel</button>
